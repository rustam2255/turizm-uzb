...
const BankCard: React.FC<{ bank: Bank; lang: Lang }> = ({ bank, lang }) => {
  ...
  return (
    <div
      className="flex flex-col w-full h-[300px] md:h-[300px] bg-white  transition-shadow duration-300 border border-[#4DC7E8]/10 overflow-hidden cursor-pointer"
      onClick={() => navigate(`/services/tour-bus/${bank.id}-${slugify(bank.name)}`)}
    >
      <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="">&gt;</span>
        <span className="text-sky-900 font-semibold">{t("services.tourbus")}</span>
      </div>
