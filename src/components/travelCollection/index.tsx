...
const TravelCollection: React.FC = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && currentSlide < tours.length - 1) {
  ...
  interface Tour {
    id: number;
    name: string;
    image?: Array<{ photo: string }>;
    city: City;
  }

  const TourCard = ({ destination, index }: { destination: Tour; index: number }) => {
    ...
    return (
      ...
      >
        <div className="bg-white dark:bg-transparent dark:border-blue-950 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 group-hover:border-blue-200">
  ...
  return (
    <>
      {tours.length > 0 && (
        <div className="py-2 lg:py-5" id="destination">
          <div className="max-w-[1900px] mx-auto px-2 sm:px-2 lg:px-8">
            ...
            <div className="hidden lg:grid lg:grid-cols-3 lg:gap-6">
              {tours.map((destination, index) => (
                <div key={destination.id}>
                  <TourCard destination={destination} index={index} />
                </div>
              ))}
            </div>
                ...
                <>
                  ...
                  {tours.length > 1 && (
                    <>
                      ...
                      >
                        <svg
                          className={`w-5 h-5 ${currentSlide === tours.length - 1 ? 'text-gray-400' : 'text-gray-700 dark:text-white'}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
            ...
            {/* Empty State */}
            {tours.length === 0 && !isLoading && !error && (
              <div className="text-center py-20">
                <div className="text-sky-400 text-lg">Hozircha turlar mavjud emas</div>
              </div>
            )}
