...
const ResortHome: React.FC = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && currentSlide < resortItems.length - 1) {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % resortItems.length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + resortItems.length) % resortItems.length);
  };
      ...
      <div className="max-w-[1900px] mx-auto px-2 sm:px-2 lg:px-8">
        ...
        <div className="hidden lg:grid lg:grid-cols-4 lg:gap-6">
          {resortItems.map((item, index) => (
            <div key={item.id}>
              <ResortCard item={item} index={index} />
            </div>
          ))}
        </div>
              ...
              {resortItems.length > 1 && (
                <>
                  ...
                  <button
                    ...
                    disabled={currentSlide === resortItems.length - 1}
                  >
                    <svg
                      className={`w-5 h-5 ${currentSlide === resortItems.length - 1 ? 'text-gray-400' : 'text-gray-700 dark:text-white'}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              {resortItems.length > 1 && (
                <div className="mt-4 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${((currentSlide + 1) / resortItems.length) * 100}%` }}
                  />
                </div>
              )}
            </>
