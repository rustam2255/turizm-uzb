...
const ShopHome: React.FC = () => {
  const { data: shopsData = { results: [] }, isLoading, isError } = useGetShopsQuery({ page: 1 });
  ...
  useEffect(() => {
    if (shopsData && !isLoading && !isError) {
      setShopItems(shopsData.results.slice(0, 4));
    }
  }, [shopsData, isLoading, isError]);
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && currentSlide < shopItems.length - 1) {
  ...
  const ShopCard = ({ item, index }: { item: Shop; index: number }) => (
    <Link
      to={`/services/market/${item.id}-${slugify(item.name)}`}
    ...
    <>
      {shopItems.length > 0 && (
        <div className="py-2 sm:py-2 lg:py-5">
          <div className="max-w-[1900px] mx-auto px-2 sm:px-2 lg:px-8">
            <div className="text-center mb-12 sm:mb-16">
              <h2 className="text-3xl sm:text-sm lg:text-3xl font-bold text-[rgba(25,110,150,255)] mb-4">
                {t("market.title")}
              </h2>
                ...
                <>
                  ...
                  {shopItems.length > 1 && (
                    <>
                      <button
                        onClick={prevSlide}
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-3 rounded-ful shadow-lg hover:bg-white dark:hover:bg-gray-700 transition-all duration-200 z-10"
                      ...
                      <button
                        ...
                        disabled={currentSlide === shopItems.length - 1}
                      >
                        <svg
                          className={`w-5 h-5 ${currentSlide === shopItems.length - 1 ? 'text-gray-400' : 'text-gray-700 dark:text-white'}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                  ...
                  {shopItems.length > 1 && (
                    <div className="mt-4 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${((currentSlide + 1) / shopItems.length) * 100}%` }}
                      />
                    </div>
                  )}
                </>
            ...
            {/* Empty State */}
            {shopItems.length === 0 && !isLoading && !isError && (
              <div className="text-center py-20">
                <div className="text-sky-400 text-lg">Hozircha do'konlar mavjud emas</div>
              </div>
            )}
            ...
            {shopItems.length > 0 && (
              <div className="text-center mt-12">
                <Link
                  to="/services/market"
                  className="inline-flex items-center md:px-8 md:py-3 px-2 py-1 bg-gradient-to-r from-sky-900 to-cyan-600 text-white font-semibold  hover:from-sky-900 hover:to-cyan-700 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl"
                >
                  <span>{t("market.see")}</span>
