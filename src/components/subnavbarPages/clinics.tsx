...
import { useGetClinicsQuery } from '@/services/api';
import { Clinic } from '@/interface';
...
const ClinicHome: React.FC = () => {
  ...
  useEffect(() => {
    if (clinicsData && !isLoading && !isError) {
      setClinicItems(clinicsData.results.slice(0, 4));
    }
  }, [clinicsData, isLoading, isError]);
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && currentSlide < clinicItems.length - 1) {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % clinicItems.length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + clinicItems.length) % clinicItems.length);
  };
  ...
  const ClinicCard = ({ item, index }: { item: Clinic; index: number }) => (
    <Link
      to={`/services/clinic/${item.id}-${slugify(item.name)}`}
